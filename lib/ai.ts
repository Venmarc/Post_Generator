...
function getModelIdentifier(model: AIModel): string {
  ...
  switch (model) {
    case 'gemini-3-pro': return 'gemini-1.5-pro'; 
    case 'gemini-3-flash': return 'gemini-2.0-flash';
    case 'gemini-2.5-pro': return 'gemini-1.5-pro';
    case 'gemini-2.5-flash': return 'gemini-1.5-flash';
...
3. NO CLICHÉS. Output MUST be ONLY a raw JSON array of 5 strings. Just the array.`;
  ...
  // Check if it's a Gemini model (identified by absence of 'gpt-')
  if (!modelId.startsWith('gpt-')) {
    try {
      // @google/genai (Preview) v1.46.0 usage
