  ...
  return (
    <div className="animate-in fade-in duration-500">
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-xl md:text-2xl font-semibold text-white mb-1 uppercase tracking-[2px]">Artenova System Overview</h1>
          <p className="text-foreground/50 text-[10px]">Real-time performance and asset telemetry</p>
        </div>
        {lastActivity && (
      ...
      <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 transition-opacity duration-300 ${loading ? 'opacity-50' : 'opacity-100'}`}>
        <div className="bg-surface rounded-card border border-border-subtle/50 p-5 shadow-card backdrop-blur-[2px] transition-all duration-300 ease-out hover:shadow-card-hover hover:-translate-y-2 hover:scale-[1.01] group">
          <div className="flex justify-between items-start mb-4">
            <div className="w-10 h-10 rounded-xl bg-accent/10 flex items-center justify-center text-accent group-hover:shadow-[0_0_15px_rgba(16,185,129,0.3)] transition-all">
          ...
          <p className="text-text-secondary text-xs">Total Assets Generated</p>
        </div>
        ...
        <div className="bg-surface rounded-card border border-border-subtle/50 p-5 shadow-card backdrop-blur-[2px] transition-all duration-300 ease-out hover:shadow-card-hover hover:-translate-y-2 hover:scale-[1.01] group">
          ...
          <p className="text-text-secondary text-xs">Export Efficiency</p>
        </div>
        ...
        <div className="bg-surface rounded-card border border-border-subtle/50 p-5 shadow-card backdrop-blur-[2px] transition-all duration-300 ease-out hover:shadow-card-hover hover:-translate-y-2 hover:scale-[1.01] group">
          ...
          <h3 className="text-base font-bold text-text-primary mb-1 truncate">{user.email}</h3>
      ...
      {!loading && generated === 0 ? (
        <div className="bg-surface rounded-card border border-border-subtle/50 p-12 shadow-card backdrop-blur-[2px] min-h-[400px] flex items-center justify-center flex-col text-center relative overflow-hidden group">
          ...
          <p className="text-text-secondary text-base max-w-md mb-6">Head over to the Create Pipeline to generate your first multimodal presentation board.</p>
        ...
        <div className="space-y-4">
          <div className="bg-surface rounded-card border border-border-subtle/50 p-5 shadow-card">
             ...
             {history.length === 0 && !loading ? (
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                 {history.map((item) => (
                    <div key={item.id} className="flex items-center justify-between p-4 bg-void/40 hover:bg-void/60 rounded-2xl border border-border-subtle/30 hover:border-accent/30 transition-all group overflow-hidden">
